...
Если проверить 8l случайных многочленов, то ни один из них не окажется 
неприводимым с вероятностью, близкой к\n
	(1 - 1/l)^8l \approx e^{-8} \approx 0.00034.

В функции belsGenM0() неуспех при 8l попытках генерации интерпретируется 
...
В функциях belsGenMi(), belsGenMid() предпринимается три попытки генерации f.
Ситуация, когда все попытки завершены с ошибкой, считается невозможной и 
интерпретируется как неверные входные данные (общему открытому ключу m0 
соответствует многочлен f0, который не является неприводимым) с возвратом 
кода ошибки ERR_BAD_PUBKEY.
