...
*******************************************************************************
\file zz.h
\brief Операции с большими целыми числами
*//****************************************************************************
...
todo: time-uniform версии функций (время выполнения не зависит от
значений входных переменных, а только от их размерностей)
*******************************************************************************
*/
...
/*!	\brief Сложение чисел
	...
	\warning Нерегулярная функция. Имеется регулярная версия zzAddSafe().
...
/*!	\brief Почти-обращение по модулю
	...
	\code
		b <- a^{-1} * 2^k \mod mod,
	\endcode
	где wordBitSize(mod) <= k <= 2 * wordBitSize(mod).
	\pre mod -- нечетное && mod[n - 1] != 0.
	\pre 0 < a < mod.
	\pre Буфер b не пересекается с буфером mod.
	\expect \gcd(a, mod) == 1.
